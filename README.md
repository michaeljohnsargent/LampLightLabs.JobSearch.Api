...
**claude-code-review GitHub Action reviews every PR against this repo's own conventions** - `.github/workflows/claude-code-review.yml` runs `anthropics/claude-code-action` on every PR open/sync and posts a review comment. The prompt explicitly points it at this repo's `CLAUDE.md` — versioning structure, the auth schemes, the Strategy Pattern used for `ICsvReaderService`/`IJobStore`, the characterization-testing rule for `StatusCategorizerService`, and the async job/DI-scope pattern in `JobsController` — so review feedback is scoped to deviations from patterns already established here, not generic style nits. It's read-only (`Read,Grep,Glob`, plus `Bash(gh pr diff:*)`/`Bash(gh pr view:*)` via `--allowedTools`) and comment-only: it cannot push commits. Pinned to `claude-sonnet-5` with `--max-turns 20`. Requires an `ANTHROPIC_API_KEY` repository secret, added once in GitHub settings.

**Why the review action is pinned to Sonnet with a turn cap and `gh pr diff` access** - The first four runs of this workflow had no model pin (defaulting to the priciest available tier) and no cheap way to see the PR diff — `--allowedTools` only granted `Read`/`Grep`/`Glob`, so the agent had no `git`/`gh` access to the changeset and fell back to exhaustively reading files while repeatedly attempting disallowed tools. Result: 16–22 permission denials and 54–68 turns per run, $2–3 each, one run failing outright. The fix mirrors `anthropics/claude-code-action`'s own `examples/pr-review-comprehensive.yml`, which grants exactly `Bash(gh pr diff:*)`/`Bash(gh pr view:*)` for this reason — a PR review agent needs a cheap way to see the diff, not a license to explore the whole repo.
